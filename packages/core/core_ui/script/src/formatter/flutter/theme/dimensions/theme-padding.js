...
const generateClassData = (json, className) => {
    ...
    Object.keys(json).forEach(key => {
        if(json[key].isSource){
            ...
            let path = json[key].path.slice(4).map(x=> capitalize(camelize(x)))
            ...
            if (val.includes('bk.global')) {
                ...
                let instance = `${path.shift()}${capitalize(camelize(path.shift()))}.shared.${path.join('.')}`
...
const generateData = (json, mark, className,depth = 0) => {
    ...
    Object.keys(json).forEach(key => {
        if (json[key].isSource) {
            ...
            // data += generateData(json[key], key, className,depth + 1)
...
StyleDictionary.registerFormat({
    name: 'flutter/theme-padding',
    formatter: ({dictionary, platform, options, file}) => {
        let generatedData = generateData(dictionary.tokens.bk.padding, '',file.className, 1)
        let data = createDartFile(file.className, generatedData, file.destination,true, false, 'theme_padding')
        data += generateClasses(dictionary.tokens.bk.padding, file.className)
        return data
    }
})

            ...
            if(!key.toLowerCase().includes('hover')) {
                ...
                if (key !== 'gradient') {
                    if (json[key].original.value.includes('bk.global')) {
                        ...
                        inner += `    final ${name} = const Color(${createColor(json[key].value)});\n`
                    } else {
                        if (json[key].original.value === 'transparent') {
                            inner += `    final ${name} = Colors.transparent;\n`
                        }
                    }
                }
            }
...
const generateData = (json, mark, className, depth = 0) => {
    ...
    Object.keys(json).forEach(key => {
        if(json[key].isSource){
            data += `    final ${camelize(key)} = const Color(${createColor(json[key].value)});\n`
        } else {
...
StyleDictionary.registerFormat({
    ...
    formatter: ({dictionary, platform, options, file}) => {
        ...
        let data = createDartFile(file.className, generatedData, file.destination, true, true, 'base_color')
